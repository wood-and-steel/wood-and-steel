...
describe('Persistence Tests', () => {
  describe('1. Moves - State persists after moves', () => {
    ...
    test('second player cannot claim hub city already claimed by another player', async () => {
      const gameCode = await createNewGame();
      ...
      useGameStore.setState({
        G: {
          ...useGameStore.getState().G,
          players: [
            ['0', { name: 'Player 0', activeCities: ['New York', 'Chicago'], hubCity: 'Chicago' as const, regionalOffice: null }],
            ['1', { name: 'Player 1', activeCities: ['Philadelphia'], hubCity: null, regionalOffice: null }],
          ],
        },
        ctx: {
          ...useGameStore.getState().ctx,
      ...
      claimHubCity('Chicago'); // Player 1 tries to claim Chicago (already hub of Player 0)
